...
public class SavedSchedulingAnnotationBeanPostProcessor implements ScheduledTaskHolder, MergedBeanDefinitionPostProcessor, DestructionAwareBeanPostProcessor,
     ...
     */
    public SavedSchedulingAnnotationBeanPostProcessor() {
     ...
     * @since 5.1
     */
    public SavedSchedulingAnnotationBeanPostProcessor(ScheduledTaskRegistrar registrar) {
        ...
        this.annotatedTasks.forEach((declaredBean, tasks)-> {
            Set<ScheduledTask>  scheduledTasks = tasks.stream().map(t -> {
                ...
                if(t instanceof FixedDelayTask) {
                    return this.registrar.scheduleFixedDelayTask((FixedDelayTask)t);
                }
                if(t instanceof CronTask) {
                    return this.registrar.scheduleCronTask((CronTask)t);
                }
                return this.registrar.scheduleTriggerTask((TriggerTask) t);
    ...
    protected void processScheduled(Scheduled scheduled, Method method, Object bean) {
        try {
            ...
            Set<Task> tasks = new LinkedHashSet<>(4);
            ...
            if (StringUtils.hasText(cron)) {
                ...
                if (StringUtils.hasLength(cron)) {
                    ...
                    if (!Scheduled.CRON_DISABLED.equals(cron)) {
                        ...
                        tasks.add(new CronTask(runnable, new CronTrigger(cron, timeZone)));
                    }
                }
            }
            ...
            if (fixedDelay >= 0) {
                ...
                processedSchedule = true;
                tasks.add(new FixedDelayTask(runnable, fixedDelay, initialDelay));
            }
            ...
            if (fixedRate >= 0) {
                Assert.isTrue(!processedSchedule, errorMessage);
                processedSchedule = true;
                tasks.add(new FixedRateTask(runnable, fixedRate, initialDelay));
            }
